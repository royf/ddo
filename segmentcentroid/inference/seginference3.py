...
class ForwardBackward(object):
    ...
    def fitTraj(self, X, max_iters=50, learning_rate=0.01):
        ...
        for it in range(0, max_iters):
            ...
            print("Sub-Iteration ", it, self.Q, np.argmax(self.B, axis=1))

            self.forward()

            self.backward()

            #print(self.fq.shape, self.bq.shape)
            ...
            for hp in range(self.k):
                ...
                forward_dict[(cur_time+1, hp)] = \
                            logsumexp([ forward_dict[(cur_time, h)] + \
                                        np.log(self._pi_a_giv_s(state,action,self.policies[h])) + \
                                        np.log(self.P[hp,h]) + 
                                        np.log(self._pi_term_giv_s(next_state,self.transitions[h]) + \
            ...
            for h in range(self.k):
                ...
                backward_dict[(cur_time-1, h)] = \
                    logsumexp([ backward_dict[(cur_time, hp)] +\
                                np.log(self._pi_a_giv_s(state,action,self.policies[h])) +\
                                np.log((self.P[hp,h]*self._pi_term_giv_s(next_state,self.transitions[h]) + \
